...
if (!empty($_FILES['profile_image']['name']) && $_FILES['profile_image']['error'] === 0) {
    ...
    // sanitize filename
    $filename = time() . "_" . preg_replace("/[^a-zA-Z0-9_\-\.]/", "_", $_FILES['profile_image']['name']);
    ...
    if ($role === "admin") {
        ...
        $stmt = $conn->prepare("INSERT INTO user (Name, Photo, Password) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $name, $photo, $hashedPassword);
        $stmt->execute();
    ...
    //Insert into account
    $sqlAcc = "INSERT INTO account (User_ID, Name, Email, Age, Role, Password, Photo, Status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmtAcc = $conn->prepare($sqlAcc);
    $stmtAcc->bind_param("ississss", $user_id, $name, $email, $age, $role, $hashedPassword, $photo, $status);
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-lg-6 col-12 p-5">
                ...
                <form action="sign-up.php" method="POST" enctype="multipart/form-data" class="p-4 border-0 rounded">
                    <div class="mb-3">
